...
// of src/nami/csrc/scalar2d_born.cu (a scattered wavefield driven by
...
// constant offsets (k*ny_nx folds, c2[k] becomes a constant-offset load), which
...
#pragma unroll
                    ...
                    const T dwfcscdz = diff1_z<T, FD_PAD>(u_sc_cur, off, c1, rdz, ny_nx);
                    const T d2zsc = diff2_z<T, FD_PAD>(u_sc_cur, off, c2, rdz2, ny_nx);
                    T dpsisc = (T)0;
#pragma unroll
                    for (int k = 1; k <= FD_PAD; ++k)
                    ...
                    wsc_sum += diff2_z<T, FD_PAD>(u_sc_cur, off, c2, rdz2, ny_nx);
                }
...
#pragma unroll
            ...
            for (int s = 0; s < n_shots; ++s) {
                ...
                const T v_val = vs[off_base];
                const T v2dt2 = v_val * v_val * dt2;
                T w_sum = diff2_z<T, FD_PAD>(u_cur, off, c2, rdz2, ny_nx)
                        + diff2_y<T, FD_PAD>(u_cur, off, c2, rdy2, nx)
                        + diff2_x<T, FD_PAD>(u_cur, off, c2, rdx2);
                u_new[off] = v2dt2 * w_sum + (T)2 * u_cur[off] - u_prev[off];
                T wsc_sum = diff2_z<T, FD_PAD>(u_sc_cur, off, c2, rdz2, ny_nx)
                          + diff2_y<T, FD_PAD>(u_sc_cur, off, c2, rdy2, nx)
...
__global__ void step_adjoint_kernel(
    ...
    int pml_z0_b, int pml_z1_b, int pml_y0_b, int pml_y1_b,
    int pml_x0_b, int pml_x1_b,
...
#define LAUNCH_STEP(KERN, T, FP, ...)                                          \
    {                                                                          \
        ...
        TORCH_CHECK(cudaGetLastError() == cudaSuccess, "nami scalar3d_born kernel failed"); \
    }
...
#define LAUNCH_FORWARD(T, ...)                                                   \
    {                                                                            \
        switch ((int)fd_pad) {                                                   \
            ...
            default: TORCH_CHECK(false, "nami scalar3d_born: unsupported fd_pad"); \
        }                                                                        \
    }
    ...
    int64_t v_batched, int64_t scatter_batched, int64_t snap_off, int64_t fd_pad)
{
    ...
    const int n_shots = lam_bg_next.size(0), nz = lam_bg_next.size(1),
              ny = lam_bg_next.size(2), nx = lam_bg_next.size(3);
    CHECK_CONTIG(c1);
    CHECK_CONTIG(c2);
    if (v.scalar_type() == torch::kFloat32) {
...
void forward_loop(
    torch::Tensor v, torch::Tensor scatter,
    std::vector<torch::Tensor> u,        // ring of 3 (background)
    std::vector<torch::Tensor> u_sc,     // ring of 3 (scattered)
    std::vector<torch::Tensor> psi_z, std::vector<torch::Tensor> psi_y,
    std::vector<torch::Tensor> psi_x,    // rings of 2 (background)
    std::vector<torch::Tensor> zeta_z, std::vector<torch::Tensor> zeta_y,
    std::vector<torch::Tensor> zeta_x,
    std::vector<torch::Tensor> psi_z_sc, std::vector<torch::Tensor> psi_y_sc,
    std::vector<torch::Tensor> psi_x_sc,  // rings of 2 (scattered)
    std::vector<torch::Tensor> zeta_z_sc, std::vector<torch::Tensor> zeta_y_sc,
    std::vector<torch::Tensor> zeta_x_sc,
    torch::Tensor az, torch::Tensor bz, torch::Tensor dbzdz,
    torch::Tensor ay, torch::Tensor by, torch::Tensor dbydy,
    torch::Tensor ax, torch::Tensor bx, torch::Tensor dbxdx,
    torch::Tensor w_store, torch::Tensor wsc_store,
    torch::Tensor f_bg, torch::Tensor f_sc, torch::Tensor src_i,
    torch::Tensor r_bg, torch::Tensor r,
    torch::Tensor bg_rec_i, torch::Tensor rec_i,
    torch::Tensor c1, torch::Tensor c2,
    double rdz, double rdy, double rdx, double rdz2, double rdy2, double rdx2,
    double dt2,
    ...
    // layout [u, u_prev, u_sc, u_sc_prev, psi_z, psi_y, psi_x,
    //          zeta_z, zeta_y, zeta_x, psi_z_sc, psi_y_sc, psi_x_sc,
    //          zeta_z_sc, zeta_y_sc, zeta_x_sc] (same as ckpt).
    ...
    // callback(t, nt, u[t%3], u_sc[t%3]) — the wavefields are the padded
    // GPU tensors (live views, no copy).
    py::object callback, int64_t callback_frequency)
{
    TORCH_CHECK(u.size() == 3 && u_sc.size() == 3 &&
                psi_z.size() == 2 && psi_y.size() == 2 && psi_x.size() == 2 &&
                zeta_z.size() == 2 && zeta_y.size() == 2 && zeta_x.size() == 2 &&
                psi_z_sc.size() == 2 && psi_y_sc.size() == 2 && psi_x_sc.size() == 2 &&
                zeta_z_sc.size() == 2 && zeta_y_sc.size() == 2 && zeta_x_sc.size() == 2,
                "nami scalar3d_born forward_loop: bad ring sizes");
    ...
    // u(t=-1) (ring slot for (t-1)%3 at t=0), u_sc likewise, psi/zeta slot 0.
    if (init_state.has_value()) {
        ...
        zeta_y[0].copy_(c[8]);
        zeta_x[0].copy_(c[9]);
        psi_z_sc[0].copy_(c[10]);
        psi_y_sc[0].copy_(c[11]);
        psi_x_sc[0].copy_(c[12]);
        zeta_z_sc[0].copy_(c[13]);
        zeta_y_sc[0].copy_(c[14]);
        zeta_x_sc[0].copy_(c[15]);
    }
    ...
    if (final_state.has_value()) {
        ...
        c[4].copy_(psi_z[nt % 2]);
        c[5].copy_(psi_y[nt % 2]);
        c[6].copy_(psi_x[nt % 2]);
        c[7].copy_(zeta_z[nt % 2]);
        c[8].copy_(zeta_y[nt % 2]);
        c[9].copy_(zeta_x[nt % 2]);
        c[10].copy_(psi_z_sc[nt % 2]);
        c[11].copy_(psi_y_sc[nt % 2]);
        c[12].copy_(psi_x_sc[nt % 2]);
        c[13].copy_(zeta_z_sc[nt % 2]);
        c[14].copy_(zeta_y_sc[nt % 2]);
        c[15].copy_(zeta_x_sc[nt % 2]);
    }
}

void adjoint_loop(
    torch::Tensor v, torch::Tensor scatter,
    std::vector<torch::Tensor> lam_bg,   // ring of 3 (background adjoint)
    std::vector<torch::Tensor> lam_sc,   // ring of 3 (scattered adjoint)
    std::vector<torch::Tensor> psi_z, std::vector<torch::Tensor> psi_y,
    std::vector<torch::Tensor> psi_x,
    std::vector<torch::Tensor> zeta_z, std::vector<torch::Tensor> zeta_y,
    std::vector<torch::Tensor> zeta_x,
    std::vector<torch::Tensor> psi_z_sc, std::vector<torch::Tensor> psi_y_sc,
    std::vector<torch::Tensor> psi_x_sc,
    std::vector<torch::Tensor> zeta_z_sc, std::vector<torch::Tensor> zeta_y_sc,
    std::vector<torch::Tensor> zeta_x_sc,
    torch::Tensor az, torch::Tensor bz, torch::Tensor dbzdz,
    torch::Tensor ay, torch::Tensor by, torch::Tensor dbydy,
    ...
    torch::Tensor w_store, torch::Tensor wsc_store,
    torch::Tensor grad_v, torch::Tensor grad_scatter,
    torch::Tensor grad_r_bg, torch::Tensor grad_r,
    torch::Tensor bg_rec_i, torch::Tensor rec_i,
    ...
    std::vector<torch::Tensor> psi_z_sc_f, std::vector<torch::Tensor> psi_y_sc_f,
    std::vector<torch::Tensor> psi_x_sc_f,
    std::vector<torch::Tensor> zeta_z_sc_f, std::vector<torch::Tensor> zeta_y_sc_f,
    std::vector<torch::Tensor> zeta_x_sc_f,
    torch::Tensor c1, torch::Tensor c2,
    double rdz, double rdy, double rdx, double rdz2, double rdy2, double rdx2,
    ...
    c10::optional<torch::Tensor> ckpt_state)
{
    TORCH_CHECK(lam_bg.size() == 3 && lam_sc.size() == 3 &&
                ...
                psi_z_sc.size() == 2 && psi_y_sc.size() == 2 && psi_x_sc.size() == 2 &&
                zeta_z_sc.size() == 2 && zeta_y_sc.size() == 2 && zeta_x_sc.size() == 2,
                "nami scalar3d_born adjoint_loop: bad ring sizes");
    const int64_t n_shots = lam_bg[0].size(0);
    const int64_t nz = lam_bg[0].size(1), ny = lam_bg[0].size(2), nx = lam_bg[0].size(3);
    const int64_t n_src = src_i.size(1), n_rec = rec_i.size(1),
                  n_bg_rec = bg_rec_i.size(1);
    const int64_t nz_ny_nx = nz * ny * nx;
    const int64_t shot_count = n_shots * nz_ny_nx;
    ...
    auto adjoint_at = [&](int64_t t, int64_t snap_off) {
        ...
        adjoint_step(
            ...
            zeta_z_sc[(t + 1) % 2], zeta_y_sc[(t + 1) % 2], zeta_x_sc[(t + 1) % 2],
            az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
            w_store, wsc_store, grad_v, grad_scatter, c1, c2,
    ...
    TORCH_CHECK(u.size() == 3 && u_sc.size() == 3 &&
                psi_z_f.size() == 2 && psi_y_f.size() == 2 && psi_x_f.size() == 2 &&
                zeta_z_f.size() == 2 && zeta_y_f.size() == 2 && zeta_x_f.size() == 2 &&
                psi_z_sc_f.size() == 2 && psi_y_sc_f.size() == 2 && psi_x_sc_f.size() == 2 &&
                zeta_z_sc_f.size() == 2 && zeta_y_sc_f.size() == 2 && zeta_x_sc_f.size() == 2,
                "nami scalar3d_born adjoint_loop: bad replay ring sizes");
    ...
    for (int64_t k = n_seg - 1; k >= 0; --k) {
        ...
        if (s0 > 0) {
            ckpt_restore(*ckpt_state, k - 1,
                         {&u[s0 % 3], &u[(s0 + 2) % 3],
                          ...
                          &psi_z_f[s0 % 2], &psi_y_f[s0 % 2], &psi_x_f[s0 % 2],
                          &zeta_z_f[s0 % 2], &zeta_y_f[s0 % 2], &zeta_x_f[s0 % 2],
                          &psi_z_sc_f[s0 % 2], &psi_y_sc_f[s0 % 2], &psi_x_sc_f[s0 % 2],
                          &zeta_z_sc_f[s0 % 2], &zeta_y_sc_f[s0 % 2], &zeta_x_sc_f[s0 % 2]});
        } else {
            zero_buffers({&u[0], &u[1], &u[2], &u_sc[0], &u_sc[1], &u_sc[2],
                          &psi_z_f[0], &psi_z_f[1], &psi_y_f[0], &psi_y_f[1],
                          &psi_x_f[0], &psi_x_f[1], &zeta_z_f[0], &zeta_z_f[1],
                          &zeta_y_f[0], &zeta_y_f[1], &zeta_x_f[0], &zeta_x_f[1],
                          &psi_z_sc_f[0], &psi_z_sc_f[1], &psi_y_sc_f[0], &psi_y_sc_f[1],
                          &psi_x_sc_f[0], &psi_x_sc_f[1], &zeta_z_sc_f[0], &zeta_z_sc_f[1],
                          &zeta_y_sc_f[0], &zeta_y_sc_f[1], &zeta_x_sc_f[0], &zeta_x_sc_f[1]});
        }
        for (int64_t t = s0; t < s1; ++t) {
            ...
            forward_step(
                ...
                psi_z_sc_f[t % 2], psi_y_sc_f[t % 2], psi_x_sc_f[t % 2],
                zeta_z_sc_f[t % 2], zeta_y_sc_f[t % 2], zeta_x_sc_f[t % 2],
                u[(t + 1) % 3], u_sc[(t + 1) % 3],
                psi_z_f[(t + 1) % 2], psi_y_f[(t + 1) % 2], psi_x_f[(t + 1) % 2],
                zeta_z_f[(t + 1) % 2], zeta_y_f[(t + 1) % 2], zeta_x_f[(t + 1) % 2],
                psi_z_sc_f[(t + 1) % 2], psi_y_sc_f[(t + 1) % 2], psi_x_sc_f[(t + 1) % 2],
                zeta_z_sc_f[(t + 1) % 2], zeta_y_sc_f[(t + 1) % 2], zeta_x_sc_f[(t + 1) % 2],
                az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                w_store, wsc_store, c1, c2,
                rdz, rdy, rdx, rdz2, rdy2, rdx2, t, interval, dt2,
                pml_z0, pml_z1, pml_y0, pml_y1, pml_x0, pml_x1,
