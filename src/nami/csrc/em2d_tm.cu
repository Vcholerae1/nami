    ...
    int cq_batched)
{
    ...
    if (y >= FD_PAD && x >= FD_PAD && y < ny - (FD_PAD - 1) &&
        ...
        T term = (T)0;
        if (y < ny - FD_PAD) {
            const T dey_dy = diff_int_y<T, FD_PAD>(ey_store, soff, c, rdy, nx);
...
// Dispatch the compile-time-FD_RADIUS kernel variants for the runtime
// (fd_pad_y0, fd_pad_x0) = (accuracy/2, accuracy/2 - 1): combinations
// (1,0) (2,1) (3,2) (4,3) for accuracy 2/4/6/8.
#define LAUNCH_EM2D(KERN, T, ...)                                              \
    {                                                                          \
        switch ((int)fd_pad_y0) {                                              \
            ...
            default: TORCH_CHECK(false, "nami em2d_tm: unsupported fd_pad");   \
        }                                                                      \
    }
    ...
    int64_t cq_batched)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(ey.scalar_type(), "em2d_tm_step_h", [&] {
        LAUNCH_EM2D(step_h_kernel, scalar_t,
    ...
    int64_t ca_batched, int64_t cb_batched)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(ey.scalar_type(), "em2d_tm_step_e_storage", [&] {
        LAUNCH_EM2D(step_e_storage_kernel, scalar_t,
    ...
    int64_t fd_pad_y0, int64_t fd_pad_y1, int64_t fd_pad_x0, int64_t fd_pad_x1)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(lam_ey.scalar_type(), "em2d_tm_coeff_grad", [&] {
        LAUNCH_EM2D(coeff_grad_kernel, scalar_t,
    ...
    int64_t ca_batched, int64_t cb_batched)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(lam_ey.scalar_type(), "em2d_tm_adjoint_e_stage1", [&] {
        LAUNCH_EM2D(adjoint_e_stage1_kernel, scalar_t,
            ca.data_ptr<scalar_t>(), cb.data_ptr<scalar_t>(),
            lam_ey.data_ptr<scalar_t>(),
    ...
    int64_t fd_pad_y0, int64_t fd_pad_x0)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(lam_hx.scalar_type(), "em2d_tm_adjoint_e_stage2", [&] {
        LAUNCH_EM2D(adjoint_e_stage2_kernel, scalar_t,
            work_x.data_ptr<scalar_t>(), work_y.data_ptr<scalar_t>(),
    ...
    int64_t cq_batched)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(lam_hx.scalar_type(), "em2d_tm_adjoint_h_stage1", [&] {
        LAUNCH_EM2D(adjoint_h_stage1_kernel, scalar_t,
    ...
    int64_t cq_batched)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(lam_hx.scalar_type(), "em2d_tm_cq_grad", [&] {
        LAUNCH_EM2D(cq_grad_kernel, scalar_t,
            cq.data_ptr<scalar_t>(),
            lam_hx.data_ptr<scalar_t>(), lam_hz.data_ptr<scalar_t>(),
    ...
    int64_t fd_pad_y0, int64_t fd_pad_x0)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(lam_ey.scalar_type(), "em2d_tm_adjoint_h_stage2", [&] {
        LAUNCH_EM2D(adjoint_h_stage2_kernel, scalar_t,
            work2_x.data_ptr<scalar_t>(), work2_y.data_ptr<scalar_t>(),
            lam_ey.data_ptr<scalar_t>(),
            c.data_ptr<scalar_t>(),
            (scalar_t)rdy, (scalar_t)rdx,
...
void backward_step(
    ...
    c10::optional<torch::Tensor> grad_f, torch::Tensor src_i,
    torch::Tensor ey_store, torch::Tensor curl_store,
    torch::Tensor grad_ca, torch::Tensor grad_cb,
    c10::optional<torch::Tensor> grad_cq,
    torch::Tensor ay, torch::Tensor by, torch::Tensor ax, torch::Tensor bx,
    torch::Tensor ky, torch::Tensor kx,
    torch::Tensor ayh, torch::Tensor byh, torch::Tensor axh, torch::Tensor bxh,
    torch::Tensor kyh, torch::Tensor kxh,
...
void forward_loop(
    ...
    // layout [ey, hx, hz, m_ey_z, m_ey_x, m_hx_z, m_hz_x] (same as ckpt).
    ...
    // callback(t, nt, ey, hx, hz) — the padded GPU tensors (live views, no
    // copy); the PML memory variables are not exposed.
    py::object callback, int64_t callback_frequency)
{
    const int64_t n_shots = ey.size(0), ny = ey.size(1), nx = ey.size(2);
    ...
    // Optional initial state (continuation runs): slot order matches the
    // N_STATE comment in em2d_tm.py.  The buffers are flat (not rings), so
    // each slot restores its buffer directly.
    ...
    for (int64_t t = 0; t < nt; ++t) {
        ...
        // checkpoint boundary (checkpointed mode only); slot order matches
        // the N_STATE comment in em2d_tm.py
        if (ckpt && t > 0 && t % checkpoint_every == 0) {
            ckpt_save(*ckpt_state, t / checkpoint_every - 1,
                      {&ey, &hx, &hz, &m_ey_z, &m_ey_x, &m_hx_z, &m_hz_x});
        }
        if (has_cb && t % callback_frequency == 0) {
            py::gil_scoped_acquire gil;
            callback(t, nt, ey, hx, hz);
        }
        ...
        forward_step(
            cq, ey, hx, hz, m_ey_z, m_ey_x, m_hx_z, m_hz_x,
            ca, cb,
            ey_store, curl_store,
            ...
            c,
            f, src_i, r, rec_i,
            rdy, rdx, t, interval, snap_off,
            pml_y0, pml_y1, pml_x0, pml_x1,
            fd_pad_y0, fd_pad_y1, fd_pad_x0, fd_pad_x1,
            cq_batched, ca_batched, cb_batched,
            n_shots, ny, nx, ny_nx, n_src, n_rec, 1);
    }

    // Optional final state (continuation): the flat buffers hold the state
    // after the last forward step (ey at t = nt, hx/hz at t = nt + 1/2, memory
    // vars at their latest values), so they are copied back directly.  This
    // makes a split run (continuation via init_state) bitwise match a one-shot
    // run.
...
void adjoint_loop(
    ...
    torch::Tensor kyh, torch::Tensor kxh,
    torch::Tensor c,
    std::vector<torch::Tensor> fwd_state,  // 7 replay buffers (empty = full storage)
...
{
    const int64_t n_shots = lam_ey.size(0), ny = lam_ey.size(1), nx = lam_ey.size(2);
    const int64_t n_src = src_i.size(1), n_rec = rec_i.size(1);
    const int64_t ny_nx = ny * nx;
    const int64_t shot_count = n_shots * ny_nx;
    ...
    auto adjoint_at = [&](int64_t t, int64_t snap_off) {
        backward_step(
            ...
            ayh, byh, axh, bxh, kyh, kxh,
            c,
            rdy, rdx, t, interval, scale, snap_off,
            pml_y0, pml_y1, pml_x0, pml_x1,
            fd_pad_y0, fd_pad_y1, fd_pad_x0, fd_pad_x1,
            cq_batched, ca_batched, cb_batched,
    ...
    // the forward steps to regenerate the ey/curl snapshots, then run the
    // adjoint.  The adjoint state carries across segments.
    TORCH_CHECK(fwd_state.size() == 7,
                "nami em2d_tm adjoint_loop: bad replay state size");
    torch::Tensor& ey_f = fwd_state[0];
    torch::Tensor& hx_f = fwd_state[1];
    torch::Tensor& hz_f = fwd_state[2];
    torch::Tensor& m_ey_z_f = fwd_state[3];
    torch::Tensor& m_ey_x_f = fwd_state[4];
    torch::Tensor& m_hx_z_f = fwd_state[5];
    torch::Tensor& m_hz_x_f = fwd_state[6];
    ...
    for (int64_t k = n_seg - 1; k >= 0; --k) {
        ...
        if (s0 > 0) {
            ckpt_restore(*ckpt_state, k - 1,
                         {&ey_f, &hx_f, &hz_f, &m_ey_z_f, &m_ey_x_f,
                          &m_hx_z_f, &m_hz_x_f});
        } else {
            zero_buffers({&ey_f, &hx_f, &hz_f, &m_ey_z_f, &m_ey_x_f,
                          &m_hx_z_f, &m_hz_x_f});
        }
        for (int64_t t = s0; t < s1; ++t) {
            const int64_t snap_off = ((t - s0) / interval) * shot_count;
            forward_step(
                cq, ey_f, hx_f, hz_f, m_ey_z_f, m_ey_x_f, m_hx_z_f, m_hz_x_f,
                ca, cb,
                ey_store, curl_store,
                ayh, byh, axh, bxh, kyh, kxh,
                ay, by, ax, bx, ky, kx,
                c,
                f, src_i, src_i, src_i,  // dummy record args (do_record = 0)
                rdy, rdx, t, interval, snap_off,
                pml_y0, pml_y1, pml_x0, pml_x1,
                fd_pad_y0, fd_pad_y1, fd_pad_x0, fd_pad_x1,
                cq_batched, ca_batched, cb_batched,
